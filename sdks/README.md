...
```python
from marmotgraph_python import KG

kg_client: KG.Client = KG.kg().build()
```
...
<sub>Python and Java</sub>
```python
KG.kg().host("core.kg-ppd.ebrains.eu").build()
```
...
<sub>Python</sub>
```python
KG.kg().with_device_flow(client_id="your-client-id").build()
```
...
<sub>Python</sub>
```python
KG.kg().with_token(token="your-access-token").build()
```
...
<sub>Python</sub>
```python
KG.kg().with_credentials(client_id="your-client-id", client_secret="your-client-secret").build()
```
...
<sub>Python</sub>
```python
KG.kg().with_custom_token_provider(lambda: some_way_to_fetch_the_token()).build()
```
...
<sub>Python</sub>
```python
result: ResultPage[Instance] = kg_client.instances.list("https://openminds.ebrains.eu/core/Person").invoke()
```
...
One of the main tasks you will meet when working with the KG is to iterate lists of results. 
To make your life easier, there are some convenience methods that allow you to iterate the results page by page to prevent extensive memory consumption. 
Since there are different means for the different programming languages available, we present you the different approaches. 
They have in common that the reloading of pages 

#### Python

##### The for loop
In python, The easiest way to iterate a ResultPage is to use the inbuilt iterator which allows you to use the "for loop":
