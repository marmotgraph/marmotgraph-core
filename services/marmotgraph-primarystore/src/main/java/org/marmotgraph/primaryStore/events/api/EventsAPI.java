...
package org.marmotgraph.primaryStore.events.api;
...
@AllArgsConstructor
@Component
public class EventsAPI implements Events.Client {
    ...
    public InstanceId postEvent(Event event) {
        ...
        } catch (FailedEventException e) {
            eventService.saveFailedEvent(e.getEvent(), e);
            throw e;
        } catch (Exception e) {
            PersistedEvent failedEvent = new PersistedEvent(event, authContext.getUserWithRoles().getUser(), event.getSpaceName());
            eventService.saveFailedEvent(failedEvent, e);
            throw new FailedEventException(failedEvent, e);
        }
    }
