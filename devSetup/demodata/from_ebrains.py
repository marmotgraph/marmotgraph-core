...
skip_until = "https://openminds.ebrains.eu/sands/ParcellationEntity"
#skip_types = ["https://openminds.ebrains.eu/core/File", "https://openminds.ebrains.eu/core/Hash", "https://openminds.ebrains.eu/core/Organization", "https://openminds.ebrains.eu/core/QuantitativeValue"]
skip_types = ["https://openminds.ebrains.eu/core/File", "https://openminds.ebrains.eu/core/Hash"]
...
target_kg = MarmotGraph.client(endpoint).with_credentials(sa_client_id, sa_client_secret).build(Stage.IN_PROGRESS)
...
for t in ebrains_kg.types.list(stage=Stage.IN_PROGRESS).items():
    ...
    if not do_skip:
        counter = 0
        page = ebrains_kg.instances.list(t.identifier, stage=Stage.IN_PROGRESS, pagination=Pagination(size=200))
        while page and page.data:
