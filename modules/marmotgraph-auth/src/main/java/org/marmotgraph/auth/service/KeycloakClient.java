...
import org.marmotgraph.auth.models.IssuerInfo;
import org.marmotgraph.auth.models.OpenIdConfig;
...
public class KeycloakClient {
    ...
    public KeycloakClient(JsonAdapter jsonAdapter, KeycloakConfig config) {
        this.config = config;
    ...
    public void initialize() {
        openIdConfig = loadWithRetry(0, i-> {
            String result = webClient.get().uri(config.getConfigUrl()).accept(MediaType.APPLICATION_JSON).retrieve().bodyToMono(String.class).block();
            return jsonAdapter.fromJson(result, OpenIdConfig.class);
        });
        issuerInfo = loadWithRetry(0, i -> {
            String result = webClient.get().uri(openIdConfig.getIssuer()).accept(MediaType.APPLICATION_JSON).retrieve().bodyToMono(String.class).block();
    ...
    public Map<String, Object> getUserInfo(String token){
        try {
            return webClient.get().uri(this.openIdConfig.getUserInfoEndpoint()).accept(MediaType.APPLICATION_JSON).header("Authorization", token).retrieve().bodyToMono(Map.class).block();
        }
