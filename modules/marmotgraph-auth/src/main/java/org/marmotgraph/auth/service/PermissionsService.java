...
import org.marmotgraph.auth.repositories.PermissionsRepository;
...
public class PermissionsService {
    ...
    public JsonLdDoc getClaimForRole(RoleMapping.Role role) {
        Optional<Permission> byId = permissionsRepository.findById(role.name());
    ...
    public JsonLdDoc addClaimToRole(RoleMapping.Role role, Map<String, Object> claimPattern) {
        Optional<Permission> byId = permissionsRepository.findById(role.name());
        Permission p;
        JsonLdDoc document;
        if(byId.isEmpty()) {
        ...
        boolean empty = synchronizeMaps(role.name(), claimPattern, document, false);
        if (empty) {
            permissionsRepository.deleteById(role.name());
            return null;
        } else {
    ...
    // Although we might be able to return an empty JsonLdDoc, we think it's more readable to return null
    public JsonLdDoc removeClaimFromRole(RoleMapping.Role role, Map<String, Object> claimPattern) {
        Optional<Permission> byId = permissionsRepository.findById(role.name());
        if (byId.isEmpty()) {
        ...
        boolean empty = synchronizeMaps(role.name(), claimPattern, jsonLdDoc, true);
        if (empty) {
            permissionsRepository.deleteById(role.name());
            return null;
        } else {
