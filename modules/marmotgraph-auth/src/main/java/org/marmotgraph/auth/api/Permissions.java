...
import org.marmotgraph.commons.model.auth.PermissionForFunctionality;
...
public class Permissions {
    ...
    private Set<FunctionalityInstance> getExpectedFunctionalityList(Functionality functionality, SpaceName space, UUID id) {
        ...
        if (functionality.getAllowedPermissionLevels().contains(PermissionForFunctionality.Level.GLOBAL)) {
            instances.add(new FunctionalityInstance(functionality, null, null));
        }
        if (space != null && functionality.getAllowedPermissionLevels().contains(PermissionForFunctionality.Level.SPACE)) {
            instances.add(new FunctionalityInstance(functionality, space, null));
        }
        if (id != null && functionality.getAllowedPermissionLevels().contains(PermissionForFunctionality.Level.INSTANCE)) {
    ...
    public Set<SpaceName> getSpacesForPermission(Collection<SpaceName> spaces, UserWithRoles userWithRoles, Functionality functionality) {
        ...
        final Set<FunctionalityInstance> applicableWildcardRoles = permissions.stream().filter(i -> i.id() == null && i.space() != null && i.space().isWildcard() && i.functionality() == functionality).collect(Collectors.toSet());
        final Stream<SpaceName> directHits = permissions.stream().filter(f -> f.id() == null && f.functionality().equals(functionality) && spaces.contains(f.space())).map(FunctionalityInstance::space).filter(Objects::nonNull);
        if(!applicableWildcardRoles.isEmpty()) {
            final Stream<SpaceName> spacesFromWildcards = spaces.stream().filter(space -> applicableWildcardRoles.stream().anyMatch(w -> w.space().matchesWildcard(space)));
    ...
    public Set<UUID> getInstancesWithExplicitPermission(List<FunctionalityInstance> permissions, Functionality functionality) {
        return permissions.stream().filter(f -> f.functionality().equals(functionality)).map(FunctionalityInstance::id).filter(Objects::nonNull).collect(Collectors.toSet());
    }
