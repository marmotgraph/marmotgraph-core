...
public class QuerySpecification {
    ...
    public static class PathDeserializer extends ValueDeserializer<List<Path>> {

        @Override
        public List<Path> deserialize(JsonParser jsonParser, DeserializationContext ctxt) throws JacksonException {
            final TreeNode treeNode = jsonParser.readValueAsTree();
            if(treeNode.isArray()){
                List<Path> result = new ArrayList<>();
                for(tools.jackson.databind.JsonNode n : ((JsonNode)treeNode)){
        ...
        private Path deserializePathItem(JsonParser jsonParser, TreeNode treeNode) {
            if(treeNode.isValueNode()){
                ...
                return jsonParser.objectReadContext().treeAsTokens(treeNode).readValueAs(Path.class);
            }
            return null;
        }
