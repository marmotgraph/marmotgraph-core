...
import org.marmotgraph.auth.api.InvitationAPI;
import org.marmotgraph.auth.api.Permissions;
import org.marmotgraph.commons.exceptions.*;
...
import org.marmotgraph.primaryStore.api.EventsAPI;
import org.marmotgraph.primaryStore.api.InstancesAPI;
import org.marmotgraph.primaryStore.api.ScopeAPI;
...
public class CoreInstanceController {
    ...
    private final InstancesAPI instances;
    private final ScopeAPI scopes;
    ...
    private void enrichWithPermissionInformation(DataStage stage, Collection<Result<NormalizedJsonLd>> documents) {
        ...
        documents.forEach(result -> {
                    ...
                    if (doc != null) {
                        ...
                        Set<Functionality> functionalities = permissions.stream().filter(p -> Functionality.FunctionalityGroup.INSTANCE == p.functionality().getFunctionalityGroup() && stage != null && stage == p.functionality().getStage()).filter(p -> p.appliesTo(sp, doc.idAsUUID())).map(FunctionalityInstance::functionality).collect(Collectors.toSet());
    ...
    private void enrichWithPermissionInformation(DataStage stage, ScopeElement scopeElement, List<FunctionalityInstance> permissions) {
        ...
        scopeElement.setPermissions(permissions.stream().filter(p -> Functionality.FunctionalityGroup.INSTANCE == p.functionality().getFunctionalityGroup() && stage != null && stage == p.functionality().getStage()).filter(p -> p.appliesTo(sp, scopeElement.getId())).map(FunctionalityInstance::functionality).collect(Collectors.toSet()));
