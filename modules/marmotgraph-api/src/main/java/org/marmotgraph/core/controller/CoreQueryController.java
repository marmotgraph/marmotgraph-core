...
public class CoreQueryController {
    ...
    public PaginatedStream<? extends JsonLdDoc> executeQuery(MarmotGraphQuery query, Map<String, String> params, PaginationParam paginationParam) {
        ...
        if (paginatedQueryResult != null) {
            if (paginatedQueryResult.responseVocab() != null) {
                final String responseVocab = paginatedQueryResult.responseVocab();
                final Stream<NormalizedJsonLd> stream = paginatedQueryResult.stream().getStream().peek(s -> s.applyVocab(responseVocab));
                return new PaginatedStream<>(stream, paginatedQueryResult.stream().getTotalResults(), paginatedQueryResult.stream().getSize(), paginatedQueryResult.stream().getFrom());
            }
            return paginatedQueryResult.stream();
        }
        return null;
    }
